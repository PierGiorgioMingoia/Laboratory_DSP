...
module.exports.authenticateUser = function authenticateUser(req, res, next) {
  ...
  if (req.query.type == "login") {
      ...
      .then((user) => {
        if (user === undefined) {
          ...
          if (!Users.checkPassword(user, password)) {
            ...
            Tasks.getUserActiveTask(user.id).then((task) => {
              let tmp = { userId: user.id, userName: user.name }
              if (task.tid && task.description) {
                tmp = { userId: user.id, userName: user.name, taskId: task.tid, taskName: task.description }
              }
              webSocketFunctions.addNewUser(tmp);
            }).catch((err) => {
              console.log(err)
            })
    ...
    Users.getUserByEmail(email)
      .then((user) => {
        webSocketFunctions.removeUser(user);
      }).catch((err) =>
        console.log(err)
      );
