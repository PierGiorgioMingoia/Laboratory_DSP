...
/**
 * Update the task active status
 */


const deactivateUserTask = function (userId, taskId) {
    return new Promise((resolve, reject) => {
        const sql = "UPDATE assignments SET active = ? WHERE user = ? AND NOT task = ?"
        db.run(sql, [0, userId, taskId], (err, rows) => {
            if (err) {
                reject(err)
            } else {
                resolve();
