...
class App extends React.Component {
  ...
  componentDidMount() {
    ...
    client.publish("Clietn test", "test message");

    //YOU CAN SET THE CALLBACKS FOR MQTT HERE
    client.on('connect', function () {
      console.log('client connected:' + clientId);
      client.subscribe('11', { qos: 0 });
      client.subscribe('3', { qos: 0 });
    })

    client.on('message', (topic, message, packet) => {
      console.log('Received Message:= ' + message.toString() + '\nOn topic:= ' + topic);
      this.displayTaskSelection(topic, JSON.parse(message));
    });
    //Websockets
